...
Matrix::Matrix(int rows, int cols, const double &value) {
    ...
    for(int i=0; i < this->nRows; ++i) {
        m[i] = new double[cols];
        for(int j=0; j < this->nCols; ++j) this->m[i][j] = value;
    }
}
...
void Matrix::setZeroBased(int row, int col, double value) {
    ...
    } else std::cout << "Invalid position" << std::endl;
} 
...
void Matrix::operator= (const Matrix &A) {
    ...
    this->nCols = A.getCols(); this->nRows = A.getRows();
...
bool Matrix::operator== (const Matrix& A) const {
    if (this->getRows() != A.getRows() ||
        this->getCols() != A.getCols()) return false;
    for (int i = 0; i < this->getRows(); ++i) {
        for (int j = 0; j < this->getCols(); ++j) {
...
bool Matrix::operator!= (const Matrix& A) const {
    if (this->getRows() != A.getRows() ||
        this->getCols() != A.getCols()) return true;
    for (int i = 0; i < this->getRows(); ++i) {
        for (int j = 0; j < this->getCols(); ++j) {
            if (this->getZeroBased(i, j) != A.getZeroBased(i, j))
                return true;
